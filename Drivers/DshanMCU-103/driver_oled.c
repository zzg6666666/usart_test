...
extern TaskHandle_t MyOLED_Testhandle = NULL;
void MyOLED_Test(void *argument)
{
    char printInfo[16];
    uint8_t cnt;
    char taskInfo01[] = "task01";
    char taskInfo02[] = "task02";

    OLED_Init();
    OLED_Clear();

    uint8_t bTaskIsTask01 = memcmp(taskInfo01, (char *)argument, sizeof(taskInfo01));
    uint8_t bTaskIsTask02 = memcmp(taskInfo02, (char *)argument, sizeof(taskInfo01));

    while (1)
    {
        OLED_PrintString(0, 0, (char *)argument);

        vTaskDelay(10);

        if (cnt == 99 && bTaskIsTask01 == 1)
        {
            // kill task01
            vTaskDelete(NULL);
            MyOLED_Testhandle = NULL;

            cnt++;
        }
        else if (cnt == 199 && bTaskIsTask02 == 1)
        {
            if (MyOLED_Testhandle != NULL)
            {
                OLED_ClearLine(0, 2);
                OLED_PrintString(0, 2, "kill task01");
                // kill task01
                vTaskDelete(MyOLED_Testhandle);
                MyOLED_Testhandle = NULL;
            }
        }
        else if (cnt == 0 && MyOLED_Testhandle == NULL)
        {
            OLED_ClearLine(0, 2);
            // create task01
            OLED_PrintString(0, 2, "create task01");
            xTaskCreate(MyOLED_Test, "MyOLED_TestTask01", 128, NULL, osPriorityNormal, &MyOLED_Testhandle);
        }
    }
    vTaskDelay(20);
}